...
export default function Home() {
  ...
  return (
    <div>
      ...
      <div className="relative rounded-lg overflow-hidden mb-8 -mx-4 sm:mx-0">
        <img
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-8 sm:px-12">
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">{t('home.hero.title')}</h1>
          <p className="text-white/80 mb-5 max-w-lg">{t('home.hero.subtitle')}</p>
          <Link to="/catalog" className="inline-block bg-white text-[#0B1628] font-semibold px-6 py-2.5 rounded hover:bg-white/90 transition-colors w-fit">
            {t('home.hero.cta')}
          </Link>
        </div>
      </div>
        ...
        {products.slice(0, 6).map(p => (
          <Link key={p.id} to={`/product/${p.id}`} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition group">
            ...
            <div className="p-4">
              ...
              <h3 className="font-semibold text-gray-800 mt-1 group-hover:text-[#0B1628]">{p.name}</h3>

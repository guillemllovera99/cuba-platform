...
export default function Cart() {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <Link to="/catalog" className="text-green-600 hover:underline font-medium">{t('cart.browse')}</Link>
      </div>
    )
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">{t('cart.title')} ({count()} {t('cart.items')})</h1>
      ...
      <div className="bg-white border rounded-lg p-6 mt-4 flex items-center justify-between">
        <p className="text-xl font-bold text-gray-900">{t('cart.total')}: ${total().toFixed(2)} USD</p>
        {isLoggedIn ? (

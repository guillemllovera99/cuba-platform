          ...
          {products.map(p => (
            <div key={p.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition">
              <Link to={`/product/${p.id}`}>
                {p.image_url && <img src={p.image_url} alt={p.name} className="w-full h-32 sm:h-48 object-cover" />}
                <div className="p-3 sm:p-4">
                  <span className="text-xs text-[#0B1628]/60 font-medium">{tCat(lang, p.category)}</span>
                  <h3 className="font-semibold text-gray-800 mt-1 text-sm sm:text-base line-clamp-2">{p.name}</h3>
